...
if ($pn == 1) {
    ...
    $centerPages .= '&nbsp; <a href="' . $_SERVER['PHP_SELF'] . '?location='.$location.'&distance='.$distance.'&limit='.$limit2.'&page=' . $add1 . '">' . $add1 . '</a> &nbsp;';
} else if ($pn == $lastPage) {
    $centerPages .= '&nbsp; <a href="' . $_SERVER['PHP_SELF'] . '?location='.$location.'&distance='.$distance.'&limit='.$limit2.'page=' . $sub1 . '">' . $sub1 . '</a> &nbsp;';
    $centerPages .= '&nbsp; <span class="pagNumActive">' . $pn . '</span> &nbsp;';
} else if ($pn > 2 && $pn < ($lastPage - 1)) {
    $centerPages .= '&nbsp; <a href="' . $_SERVER['PHP_SELF'] . '?location='.$location.'&distance='.$distance.'&limit='.$limit2.'page=' . $sub2 . '">' . $sub2 . '</a> &nbsp;';
    $centerPages .= '&nbsp; <a href="' . $_SERVER['PHP_SELF'] . '?location='.$location.'&distance='.$distance.'&limit='.$limit2.'page=' . $sub1 . '">' . $sub1 . '</a> &nbsp;';
    $centerPages .= '&nbsp; <span class="pagNumActive">' . $pn . '</span> &nbsp;';
    $centerPages .= '&nbsp; <a href="' . $_SERVER['PHP_SELF'] . '?location='.$location.'&distance='.$distance.'&limit='.$limit2.'page=' . $add1 . '">' . $add1 . '</a> &nbsp;';
    $centerPages .= '&nbsp; <a href="' . $_SERVER['PHP_SELF'] . '?location='.$location.'&distance='.$distance.'&limit='.$limit2.'page=' . $add2 . '">' . $add2 . '</a> &nbsp;';
...
if ($lastPage != "1"){
    ...
    if ($pn != 1) {
        $previous = $pn - 1;
        $paginationDisplay .=  '&nbsp;  <a href="' . $_SERVER['PHP_SELF'] . '?location='.$location.'&distance='.$distance.'&limit='.$limit2.'&page=' . $previous . '"> Back</a> ';
    }
    ...
    if ($pn != $lastPage) {
        $nextPage = $pn + 1;
        $paginationDisplay .=  '&nbsp;  <a href="' . $_SERVER['PHP_SELF'] . '?location='.$location.'&distance='.$distance.'&limit='.$limit2.'&page=' . $nextPage . '"> Next</a> ';
    }
}
...
foreach ($listaEventos["events"]["event"] as $key => $value) {
            $name = $value["title"];
            $url = $value["url"];
            $outputList .= '<p>' . $name . ' </><a href="'.$url.'"> link</a><hr />';
    }
...
.paginationNumbers a:active {
    ...
    <body><div class="div">
            ...
            <div align="center" class="paragrafoEstrutura">
  ...
  <script type="text/javascript">
    ...
    for (i = 0; i < locations.length; i++) {  
      ...
      google.maps.event.addListener(marker, 'click', (function(marker, i) {
        return function() {
          ...
          infowindow.open(map, marker);
            ...
            <div id="accordion"  class="divSemCor" style="height: 190px">
      ...
      <p id="mySearch">Total Items: <?php echo $nr; ?></p>
      <div><?php echo $paginationDisplay; ?></div>
      <div><?php print "$outputList"; ?></div>
      <div style="background-color: #ddd; border:#999 1px solid;"><?php echo $paginationDisplay; ?></div>
